...
type
  ...
  private
    ...
    FName: PString;
  public
    constructor Create(Colour: TColour);
    destructor Destroy; override;
...
function EnterPosition(var P: TPosition): Boolean;
var
  A, B: Char;
begin
  Result := False;
  ...
  P := MakePosition(Ord(A) - 64, Ord(B) - 48);
  Result := True;
end;
...
begin
  ...
  FPlayers[White].Create(White);
  FPlayers[Black].Create(Black);
  ...
  WriteLn('White player:');
  ReadLn(S);
  FPlayers[White].Name := S;

  WriteLn('Black player:');
  ReadLn(S);
  FPlayers[Black].Name := S;
...
procedure TBoard.WelcomeMessage;
begin
  ShowMessage('* CHESS *');
  ShowMessage('');
  ShowMessage('WDSibyl');
  ...
  ShowMessage('');
  ShowMessage('ESC to escape.');
  ShowMessage('');
  ShowMessage('Please enter');
  ShowMessage('your name.');
  ShowMessage('');
end;
...
function TBoard.EnterMove(var Start, Dest: TPosition): Boolean;
begin
  Result := False;
  Window(60, 2, 78, 24);
  TextBackground(0);
  TextColor(15);
  GotoXY(1, 23);
  WriteLn('Your move, ' + Players[CurrentPlayer].Name + ':');
  Write('> ');
  if EnterPosition(Start) then
  begin
    Write (' - ');
    if EnterPosition(Dest) then
    begin
      WriteLn;
      Result := True;
    end;
...
begin
  ...
  begin
    ...
    ShowMessage('Touche! :-)');
    ShowMessage('');
    if Victim is TKing then FGameOver := True;
...
begin
  repeat
    repeat
      if not EnterMove(A, B) then Exit;
  ...
  ShowMessage(Players[WaitingPlayer].Name + ' won.');
  ShowMessage('');
  ShowMessage('Please press <Return>.');
